        ...
        <div class="box-body">
            <div class="row">
                <div class="col-sm-6 col-sm-push-3">
                    <form role="form" method="post" action="{{route('event_beat.edit',['id' => $eventBeat->id])}}" enctype = "multipart/form-data">
                        {{ csrf_field() }}
                        @if ($errors->any())
                        ...
                        <div class="form-group{{ $errors->has('event_id') ? ' has-error' : '' }}">
                            ...
                            <select name="event_id" class="form-control" required>
                                ...
                                @foreach($events as $event)
                                    <option value="{{$event->id}}" @if($eventBeat->event_id == $event->id) selected @endif>{{$event->title}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group{{ $errors->has('action_id') ? ' has-error' : '' }}">
                            ...
                            <select name="action_id" class="form-control" required>
                                ...
                                @foreach($actions as $action)
                                    <option value="{{$action->id}}" @if($eventBeat->action_id == $action->id) selected @endif>{{$action->name}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group{{ $errors->has('points') ? ' has-error' : '' }}">
                            <label>Point</label>
                            <input name="points" type="number" class="form-control" value="{{$eventBeat->points }}" required autofocus style="width: 100px;" min="0" placeholder="Enter ...">
                            @if ($errors->has('name'))
                                <span class="help-block">
                        ...
                        <div class="form-group{{ $errors->has('comments') ? ' has-error' : '' }}">
                            ...
                            <textarea name="comments" class="form-control" placeholder="Enter..." rows="5">{{$eventBeat->comments}}</textarea>
                        </div>
